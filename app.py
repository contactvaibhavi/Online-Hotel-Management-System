...
def register():
    ...
    if request.method == "POST" and form.validate():
        ...
        # Execute query
        cur.execute(
            "INSERT INTO admins(name, email, username, password) VALUES(%s, %s, %s, %s)",
...
def add_amenity():
    ...
    if form.validate_on_submit():
        ...
        cur.execute(
            "INSERT INTO amenities(a_id, a_type, a_status, a_capacity, a_title, a_description) VALUES(%s, %s, %s, %s, %s, %s)",
            (idd, type, status, capacity, title, description),
        )
...
def edit_amenity(id):
    ...
    if form.validate_on_submit():
        ...
        cur.execute(
            "UPDATE amenities SET a_type=%s, a_status=%s, a_capacity=%s, a_title=%s, a_description=%s WHERE a_id=%s",
            (type, status, capacity, title, description, id),
        )
...
def add_room():
    ...
    if request.method == "POST" and form.validate():
        ...
        cur.execute(
            "INSERT INTO rooms(r_id, r_number, r_type, r_capacity, r_status) VALUES(%s, %s, %s, %s, %s)",
            (id, number, type, capacity, status),
        )
...
def edit_room(id):
    ...
    if request.method == "POST":  # form.validate_on_submit():
        ...
        cur.execute(
            "UPDATE rooms SET r_type=%s, r_status=%s, r_capacity=%s WHERE r_id=%s",
...
def bookings(id):
    ...
    if request.method == "POST":  # form.g_id.validate_on_submit():
        ...
        if id[0] == "R":
            ...
            result = cur.execute(
                "SELECT cost FROM charges WHERE code = 1 AND type=%s", [f_type]
            )
            result = cur.fetchone()
            f_cost = result["cost"]
            ...
            result = cur.execute(
                "SELECT cost FROM charges WHERE code = 0 AND type=%s", [f_type]
            )
            result = cur.fetchone()
            f_cost = result["cost"]
        ...
        if id[0] == "R":
            cur.execute(
                "INSERT INTO bookings(b_id, r_id, g_id, b_status, a_id, st, et, f_type, f_cost) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (b_id, id, g_id, status, "0", check_in, check_out, f_type, f_cost),
            )
            cur.execute(
                "INSERT INTO guests(g_id, g_name, g_email, g_count, g_streetno, g_city, g_state, g_country, g_pincode) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
